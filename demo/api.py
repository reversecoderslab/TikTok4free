...
def get_api_common_params(endpoint):
    if len(rapidapi_key) > 0:
        ...
        raise Exception("Please fill in your rapidapi key from https://rapidapi.com/reversecoders/api/tiktok4free.")
    return url, payload, headers


def do_get_dev_tmpl(proxy, country):
...
def encrypt_get_setting(dev_info):
    url, payload, headers = get_api_common_params("get_mscc_body")
    payload = json.dumps(payload | {
        "dev_info": dev_info
    })
    printf(payload)
    ...
    if response.status_code == 200:
        obj = json.loads(response.text)
        data = obj["data"]
        printf(f'Setting body: {data}')
        return base64.b64decode(data)


def encrypt_get_report(dev_info, mode):
